...
local struct int4d { x : int, y : int, z : int, w : int }
...
local filename = "small_test.h5"

task print_region(r : region(ispace(int4d), ImgWrapper))
where
	reads (r)
do
	for i = r.bounds.lo.x, r.bounds.hi.x do
		c.printf("%s ", r[{i,0,0,0}].cspad)
	end
	c.printf("\n")
end
...
task main()
	var ctx : ImgContext;
	ctx:init();
	var r_img = region(ispace(int4d, {ctx.EVENTS, ctx.SHOTS, ctx.WIDTH, ctx.HEIGHT}), ImgWrapper)
	var p_img = partition(equal, r_img, ispace(int4d, {5, 1, 1, 1}))
	attach(hdf5, r_img, filename, regentlib.file_read_write)
